...
**CMake targets** — 23 of them, one per `Tools/*_test.cpp` that is wired up:
...
CI runs only the eight JUCE-free targets — `WaveTest`, `DistTest`, `MasterTest`,
`DynFiltTest`, `SpaceTest`, `SongTest`, `RepeatTest`, `AttackTest` — on both
platforms, because a failure in those is a real portability bug rather than a
headless-runner artefact. The rest are a local macOS run, so they are still on you.
